import test from 'node:test';
import assert from 'node:assert/strict';

import { softDeletePatientAllergy } from './softDeletePatientAllergy.ts';

test('softDeletePatientAllergy marks an allergy deleted by id', async () => {
  const calls: Array<{ sql: string; params?: unknown[] }> = [];
  const service = softDeletePatientAllergy({
    async query<T>(sql: string, params?: unknown[]) {
      calls.push({ sql, params });
      return { rows: [{ id: 'allergy-1', deleted_at: '2026-01-01T00:00:00.000Z' }] as T[] };
    },
  });

  const result = await service({ allergyId: 'allergy-1' });

  assert.match(calls[0].sql, /UPDATE patient_allergies/);
  assert.match(calls[0].sql, /SET deleted_at = COALESCE\(deleted_at, NOW\(\)\)/);
  assert.deepEqual(calls[0].params, ['allergy-1']);
  assert.equal((result as { id: string }).id, 'allergy-1');
});
