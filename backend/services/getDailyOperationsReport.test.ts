...
test('getDailyOperationsReport builds daily clinic operations aggregate', async () => {
  ...
  const result = await service({ clinicId: 'clinic-1', startDate: '2026-05-24', endDate: '2026-05-31' });

  assert.match(calls[0].sql, /WITH visit_rows AS/);
  assert.match(calls[0].sql, /top_diagnoses/);
  assert.match(calls[0].sql, /by_prescriber/);
  assert.deepEqual(calls[0].params, ['clinic-1', '2026-05-24', '2026-05-31']);
